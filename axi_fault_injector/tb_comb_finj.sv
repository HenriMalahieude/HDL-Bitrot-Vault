...
module tb_comb_finj();
	...
	//SEUP (output) side axi
	wire [12:0] seup_awid;
	wire [31:0] seup_awaddr; //inj here
	wire [7:0] 	seup_awlen;
	wire [2:0] 	seup_awsize;
	wire [1:0] 	seup_awburst;
	wire 		seup_awvalid;
	reg			seup_awready;

	wire [31:0]	seup_wdata; //inj here
	wire [3:0]	seup_wstrb;
	wire		seup_wlast;
	wire		seup_wvalid;
	reg 		seup_wready;
	...
	wire [12:0] 	seup_arid;
	wire [31:0] 	seup_araddr; //inj here
	wire [7:0] 	    seup_arlen;
	wire [2:0] 	    seup_arsize;
	wire [1:0] 	    seup_arburst;
	wire			seup_arvalid;
	reg				seup_arready;

	reg [12:0] 		seup_rid;
	reg [31:0]		seup_rdata; //inj here
	reg [1:0] 		seup_rresp;
	reg				seup_rlast;
	reg				seup_rvalid;
	wire			seup_rready;

	//SEUP side (output) (slave)
	axi_vip_0 seup_vip(
		.aclk(clk),
		.aresetn(rstl),

		.s_axi_araddr(seup_araddr),
		.s_axi_arburst(seup_arburst),
		.s_axi_arid(seup_arid),
		.s_axi_arlen(seup_arlen),
		.s_axi_arsize(seup_arsize),
		.s_axi_arvalid(seup_arvalid),
		.s_axi_arready(seup_arready),
		.s_axi_awaddr(seup_awaddr),
		.s_axi_awburst(seup_awburst),
		.s_axi_awid(seup_awid),
		.s_axi_awlen(seup_awlen),
		.s_axi_awsize(seup_awsize),
		.s_axi_awvalid(seup_awvalid),
		.s_axi_awready(seup_awready),
		.s_axi_bid(seup_bid),
		.s_axi_bready(seup_bready),
		.s_axi_bvalid(seup_bvalid),
		.s_axi_bresp(seup_bresp),
		.s_axi_rready(seup_rready),
		.s_axi_rdata(seup_rdata),
		.s_axi_rid(seup_rid),
		.s_axi_rlast(seup_rlast),
		.s_axi_rresp(seup_rresp),
		.s_axi_rvalid(seup_rvalid),
		.s_axi_wdata(seup_wdata),
		.s_axi_wlast(seup_wlast),
		.s_axi_wstrb(seup_wstrb),
		.s_axi_wvalid(seup_wvalid),
		.s_axi_wready(seup_wready)
	);

	//begin verifier int mem mode (save the writes)
	axi_vip_0_slv_mem_t seup_vip_agent;
	initial begin
		seup_vip_agent = new("seup vip mem agent", tb_comb_finj.seup_vip.inst.IF);
		seup_vip_agent.start_slave();
	end
	...
	comb_finj #(
		...
		.m_awid(seup_awid),
		.m_awaddr(seup_awaddr),
		.m_awlen(seup_awlen),
		.m_awsize(seup_awsize),
		.m_awburst(seup_awburst),
		.m_awvalid(seup_awvalid),
		.m_awready(seup_awready),
		.m_wdata(seup_wdata),
		.m_wstrb(seup_wstrb),
		.m_wlast(seup_wlast),
		.m_wvalid(seup_wvalid),
		.m_wready(seup_wready),
		.m_bid(seup_bid),
		.m_bresp(seup_bresp),
		.m_bvalid(seup_bvalid),
		.m_bready(seup_bready),
		.m_arid(seup_arid),
		.m_araddr(seup_araddr),
		.m_arlen(seup_arlen),
		.m_arsize(seup_arsize),
		.m_arburst(seup_arburst),
		.m_arvalid(seup_arvalid),
		.m_arready(seup_arready),
		.m_rid(seup_rid),
		.m_rdata(seup_rdata),
		.m_rresp(seup_rresp),
		.m_rlast(seup_rlast),
		.m_rvalid(seup_rvalid),
		.m_rready(seup_rready),
	...
	initial begin
		...
		#(`CLK_PERIOD*16)
		$display("Injection type checks have been disabled due to parameters");
		$display("DONE");
		$finish;
	end
